...
import { userPost, postData, saveUserPost } from '../../store/fetchPostSlice';
...
import {DeletePost, olderPostData} from '../../store/savePostSlice';
import {removeElement} from '../customMethods';
...
export default function UserPost() {
	...
	const userPostArr = useSelector(postData);
	...
	const dlPost =(postId)=> {
		const newArr = removeElement(userPostArr.userPost, postId);
		dispatch(saveUserPost(newArr));
		dispatch(DeletePost(postId));
		history.push('/userposts');
	}

	const editThisPost=(path)=> {
		dispatch(olderPostData(path));
			...
			<Grid container item lg={12} spacing={1}>
				<Grid item lg={10} xs={12}>
									...
									.map((row) => (
										...
										>
											<TableCell component="th" scope="row">
												<div style={{ cursor: 'pointer' }} onClick={() => history.push('/' + row.path)} className="title">
													{row.isPassword ? (
											...
											<TableCell align="right">
												<Tooltip title="Edit this Post">
													<EditIcon onClick={() => editThisPost(row.path)} />
												</Tooltip>
												&nbsp;
												&nbsp;
												<Tooltip title="Delete this Post">
													<DeleteForeverIcon onClick={() => dlPost(row.path)} />
												</Tooltip>
											</TableCell>
										</TableRow>
									))}
					...
					<TablePagination
						...
						component="div"
						count={userPostArr.userPost.length}
